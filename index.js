...
Parser.prototype._transform = function(file, encoding, done) {
  ...
  var stringPattern =
    "(?:" + singleQuotePattern + "|" + doubleQuotePattern + "|" + backQuotePattern + ")";
  var pattern =
    "(?:\\W|^)(?:" +
    fnPattern +
    ")\\s*\\(?\\s*" +
    stringPattern +
    "(?:(?:[^).]*?)\\{(?:.*?)(?:(?:context|'context'|\"context\")\\s*:\\s*" +
    stringPattern +
    "(?:.*?)\\}))?";
  ...
  // ====================================================================
  var noStringLiteralPattern =
    "[^a-zA-Z0-9_'\"`]((?:" + fnPattern + ")(?:\\()\\s*(?:[^'\"`)]+\\)))";
  ...
  var attributeWithoutValueRegex = new RegExp(
    '<([A-Z][A-Z0-9]*)(?:(?:\\s+[A-Z0-9-]+)(?:(?:=")(?:[^"]*)(?:"))?)*(?:(?:\\s+' +
      attributes +
      '))(?:(?:\\s+[A-Z0-9-]+)(?:(?:=")(?:[^"]*)(?:"))?)*\\s*(?:>(.*?)<\\/\\1>)',
    "gi"
  );
...
Parser.prototype._flush = function(done) {
  ...
  for (var i in self.locales) {
    ...
    for (var namespace in translationsHash) {
      ...
      mergedTranslations = helpers.mergeHash(
        currentTranslations,
        translationsHash[namespace],
        null,
        this.keepRemoved
      );
      ...
      mergedTranslations.additional = _.extend(
        additionalTranslations,
        mergedTranslations.additional
      );
